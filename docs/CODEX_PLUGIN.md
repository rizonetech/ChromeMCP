...
Use the actual clone path for `source`. For WSL clones, `wslpath -w "$(pwd)"` prints the Windows UNC path.
...
- Keep `plugins/chromemcp-browser/.codex-plugin/plugin.json`, `.mcp.json`, and `skills/` together. Those files are the redistributable plugin payload.
