...
- Tested with `@playwright/mcp@0.0.76` (pinned exactly in [`mcp/package.json`](mcp/package.json); see [`CHANGELOG.md`](CHANGELOG.md) for upgrade notes)
...
This installs ChromeMCP to `~/ChromeMCP` and symlinks `chromemcp` into `~/.local/bin`. Then run:
...
chromemcp token          # print bearer token for your client config
chromemcp test           # smoke test
```

For Codex plugin install, see [`docs/CODEX_PLUGIN.md`](docs/CODEX_PLUGIN.md).
...
- **Codex** — use the Rizonetech plugin from the `rizonetech/codex-plugins` repository. See [`docs/CODEX_PLUGIN.md`](docs/CODEX_PLUGIN.md).
...
The Codex plugin wrapper lives in the `rizonetech/codex-plugins` repository. Install from PowerShell at the `codex-plugins` root:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\install-rizonetech-local.ps1
```

Then restart Codex and start ChromeMCP from WSL:
...
```
.
├── chromemcp           Unified CLI (dispatches to per-action wrappers)
...
├── bridge-check        Diagnose / repair bridge drift
├── mcp-up / mcp-down   Start / stop the MCP server
├── mcp-status          Health report (mode-aware)
├── mcp-enable / mcp-disable  Install / remove the systemd user unit
├── mcp-logs            Tail server logs (journal or file mode)
├── mcp-token           Print / rotate the bearer auth token
...
├── mcp/
│   ├── auth-proxy.js            Auth proxy / watchdog / healthz / metrics
...
│   └── tests/                   Full regression suite (~21 s, 10 test files)
└── scripts/install.sh           Installer (release tarball or --from-source)
```
...
Planned CI (tracked in [`todo/production-readiness.md`](todo/production-readiness.md)): a smoke workflow will boot a headless Chromium on `ubuntu-latest`, start the proxy with `MCP_NO_AUTH=1`, and run `run-all.sh` on every PR + push to `main`.
...
If Chrome on Windows crashes, you close it, or it restarts itself for an update, the `@playwright/mcp` upstream's CDP WebSocket dies and the upstream does not auto-reattach (verified by source inspection). ChromeMCP's auth proxy includes a watchdog that handles this:
